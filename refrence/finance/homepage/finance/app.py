...
def index():
    ...
    if request.method == "GET":
        stocks = db.execute("SELECT symbol ,shares,SUM(shares) as total_shares FROM transactions WHERE user_id = :user_id GROUP BY symbol HAVING total_shares > 0",user_id=session["user_id"])
...
def buy():
    ...
    if (item["price"]) * int(shares) > cash[0]["cash"] :
            ...
            db.execute("UPDATE users SET cash = cash - :price WHERE id = :id", price = (item["price"] * int(shares)), id = session["user_id"])
...
def sell():
    ...
    if request.method == "POST":
        symbol = db.execute("SELECT symbol FROM transactions WHERE user_id = :user_id",user_id=session["user_id"] )
        ...
        for stock in stocks :
            if stock["symbol"] == symbol:
                if stock["total_shares"]<int(shares):
                    ...
                    shares=-int(shares)
                    db.execute("UPDATE users SET cash = cash + :total_sell WHERE id = :user_id",user_id=session["user_id"],total_sell=total_sell)
                    db.execute("INSERT INTO transactions (user_id,symbol,shares,price) VALUES (?,?,?,?)",session["user_id"],symbol,shares,price)
                    return redirect("/")
    if request.method == "GET":
        symbol = db.execute("SELECT symbol FROM transactions WHERE user_id = :user_id",user_id=session["user_id"])
